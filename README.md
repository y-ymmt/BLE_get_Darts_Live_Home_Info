...
⚠️ **重要**: 現在のセグメントマッピングは推定値です。実際のDARTSLIVE HOMEデバイスで各セグメントのコードを確認し、`src/data/mapper.py` のマッピングテーブルを調整する必要があります。

### マッピングのキャリブレーション方法

1. 各セグメントに意図的にダーツを投げる
2. ログに表示されるセグメントコードを記録
3. `mapper.py` の `_create_default_mapping()` メソッドを実際のコードに合わせて修正
